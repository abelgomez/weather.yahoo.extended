...
def forecast(loc, locid, enable_sensors, prefer_sensors, sensors_url):
    ...
    if data:
        ...
        if enable_sensors:
            sensor_data = get_sensor_data(sensors_url)
        else:
            sensor_data = {}
...
def properties(response, loc, locid, prefer_sensors, sensor_data):
    ...
    if prefer_sensors and 'temperature' in sensor_data and 'humidity' in sensor_data:
        ...
        set_property('Current.Condition'         , u"{}\N{DEGREE SIGN}C \N{MIDDLE DOT} {}% \N{MIDDLE DOT} {}".format(convert_temp(data['observation']['temperature']['now']), str(data['observation']['humidity']), data['observation']['conditionDescription']))
    else:
        ...
        set_property('Current.Condition'         , data['observation']['conditionDescription'] if 'temperature' not in sensor_data and 'humidity' not in sensor_data else u"{:.1f}\N{DEGREE SIGN}C \N{MIDDLE DOT} {:.0f}% \N{MIDDLE DOT} {}".format(sensor_data['temperature'], sensor_data['humidity'], data['observation']['conditionDescription']))
